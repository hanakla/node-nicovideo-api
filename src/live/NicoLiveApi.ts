...
import {default as LiveSession, NicoLiveConnectPreference} from './LiveSession'
 ...
 * @return {Promise}
 */
export const getLiveInfo = async (session: NicoSession, liveId: string): Promise<LiveMetaData> =>
{
    ...
    if (res.statusCode === 503) {
        throw new NicoException({message: `Live[${liveId}]: Nicovideo has in maintenance.`})
    }

    return _parseLiveInfoResponse(res.body)
}

export const getLiveSession = async (session: NicoSession, liveId: LiveMetaData|string, options: Partial<NicoLiveConnectPreference> = {}): Promise<LiveSession> =>
{
    const liveMeta: LiveMetaData = typeof liveId === 'string' ? await getLiveInfo(session, liveId) : liveId
