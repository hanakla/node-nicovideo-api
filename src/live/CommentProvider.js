...
export default class CommentProvider extends Emitter {
    ...
    _fetchPostKey() {
        ...
        let threadId    = this._live.get("comment.thread");
        let url         = sprintf(NicoUrl.Live.GET_POSTKEY, threadId);
        let postKey     = "";
    ...
    postComment(msg, command = "", timeoutMs = 3000) {
        ...
        return this._fetchPostKey().then(() => {
            ...
            let disposer = this._onDidReceivePostResult(function({status}) {
                disposer.dispose();
                clearTimeout(timerId);
